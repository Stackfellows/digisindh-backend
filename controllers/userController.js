...
exports.generateAndSendPDF = async (req, res) => {
  try {
    ...
    const emailSubject =
      isSecondEnroll === "true"
        ? "Your Second Enrollment Challan is Ready - Sindhrozgar"
        : "Your Challan is Ready - Sindhrozgar";
...
exports.updateTestScore = async (req, res) => {
  try {
    ...
    // If user passed the test, send an email and generate challan
    if (updatedUser.testPassed === true) {
      // 1. Send Test Passed Email
      ...
      await sendEmail({
        ...
        subject:
          "Congratulations! You Have Passed the Admission Test – Now You Are Eligible For Sindhrozgar Scholarship Card",
        html: testPassedHtml,
        emailType: "admissions",
      });

      // 2. Generate and Send Challan
      try {
        const amount = 3250;
        const userCourses = updatedUser.admissionType.includes("physical") 
          ? updatedUser.physicalCourses 
          : updatedUser.courses;
        ...
        // Send challan email
        const challanHtml = getChallanEmailHtml({
          userName: updatedUser.fullName,
          challanNumber: challanNumber,
          amount: amount,
        });
        ...
        await sendEmail({
          email: updatedUser.email,
          subject: "Your Sindhrozgar Admission Challan is Ready",
          html: challanHtml,
          emailType: "admissions",
          attachments: [
            {
        ...
        console.error("Auto challan generation failed:", challanErr);
        // We don't return error here because the test score update was successful
      }
...
exports.adminGenerateAndSendPDF = async (req, res) => {
  try {
    ...
    const emailResult = await sendEmail({
      email: user.email,
      subject: "Your Challan is Ready - Sindhrozgar",
      html: html,
      emailType: "contact",
      attachments: [
        {
